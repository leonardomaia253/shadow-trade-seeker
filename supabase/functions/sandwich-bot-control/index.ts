...
async function startBot(supabase, config) {
  ...
  await supabase.from('bot_logs').insert({
    ...
    bot_type: 'sandwich',
    source: 'system',
    metadata: { minProfitThreshold, minSlippageThreshold, targetDEXs }
  });
...
async function updateBotConfig(supabase, config) {
  ...
  await supabase.from('bot_logs').insert({
    level: 'info',
    message: `Bot configuration updated: min profit=${minProfitThreshold} ETH, min slippage=${minSlippageThreshold}%`,
