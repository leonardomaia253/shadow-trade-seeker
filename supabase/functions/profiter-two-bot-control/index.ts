...
async function mockPm2Operation(operation, params = {}) {
  ...
  switch (operation) {
    case 'start':
      return { success: true, output: `Started process ${params.name || 'profiter-two'}` };
    case 'stop':
      return { success: true, output: `Stopped process ${params.name || 'profiter-two'}` };
    case 'restart':
      return { success: true, output: `Restarted process ${params.name || 'profiter-two'}` };
    case 'logs':
      return { success: true, output: 'mock logs output for profiter-two' };
    case 'status':
      return { success: true, status: 'online', output: 'profiter-two online' };
    default:
...
async function stopBotWithPm2(supabase) {
  ...
  const pm2Result = await mockPm2Operation('stop', { name: 'profiter-two' });
...
async function restartBotWithPm2(supabase) {
  ...
  const pm2Result = await mockPm2Operation('restart', { name: 'profiter-two' });
...
async function getPm2Logs(supabase) {
  ...
  const pm2Result = await mockPm2Operation('logs', { name: 'profiter-two', lines: 50 });
...
async function getPm2Status(supabase) {
  ...
  const pm2Result = await mockPm2Operation('status', { name: 'profiter-two' });
