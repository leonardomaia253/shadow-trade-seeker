...
const ProfiterOneBot = () => {
  ...
  useEffect(() => {
    ...
    const statsChannel = supabase
      .channel('profiter-one-stats-updates')
      .on('postgres_changes', 
          { event: 'UPDATE', schema: 'public', table: 'bot_statistics', filter: 'bot_type=eq.profiter-one' },
          (payload) => {
    ...
    const txChannel = supabase
      .channel('profiter-one-transactions-updates')
      .on('postgres_changes', 
          { event: 'INSERT', schema: 'public', table: 'bot_transactions', filter: 'bot_type=eq.profiter-one' },
          (payload) => {
  ...
  const handleStartBot = async () => {
    try {
      ...
      enhancedLogger.info("Starting profiter-one bot");
      setIsStarting(true);
      ...
      const { data, error } = await supabase.functions.invoke('profiter-one-bot-control', {
      ...
      toast({
        title: "Bot started",
        description: "Profiter One bot has been started successfully",
      });
  ...
  const handleStopBot = async () => {
    try {
      ...
      enhancedLogger.info("Stopping profiter-one bot");
      setIsStopping(true);
      ...
      toast({
        title: "Bot stopped",
        description: "Profiter One bot has been stopped successfully",
      });
