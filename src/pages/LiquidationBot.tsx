...
const LiquidationBot = () => {
  ...
  useEffect(() => {
    ...
    const statsChannel = supabase
      .channel('liquidation-stats-updates')
      .on('postgres_changes', 
          { event: 'UPDATE', schema: 'public', table: 'bot_statistics', filter: 'bot_type=eq.liquidation' },
          (payload) => {
    ...
    const txChannel = supabase
      .channel('liquidation-transactions-updates')
      .on('postgres_changes', 
          { event: 'INSERT', schema: 'public', table: 'bot_transactions', filter: 'bot_type=eq.liquidation' },
          (payload) => {
  ...
  const handleStartBot = async () => {
    ...
    try {
      ...
      toast({
        title: "Bot Started",
        description: "Liquidation bot is now running and searching for opportunities",
      });
  ...
  const handleStopBot = async () => {
    ...
    try {
      ...
      toast({
        title: "Bot Stopped",
        description: "Liquidation bot has been stopped",
      });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-crypto-darker text-foreground font-mono flex items-center justify-center">
        <div className="text-neon-purple animate-pulse">Loading bot data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-crypto-darker text-foreground font-mono">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <BotLogsViewer botType="liquidation" />
        </div>
