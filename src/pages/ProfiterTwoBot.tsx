...
const ProfiterTwoBot = () => {
  ...
  useEffect(() => {
    ...
    const statsChannel = supabase
      .channel('profiter-two-stats-updates')
      .on('postgres_changes', 
          { event: 'UPDATE', schema: 'public', table: 'bot_statistics', filter: 'bot_type=eq.profiter-two' },
          (payload) => {
    ...
    const txChannel = supabase
      .channel('profiter-two-transactions-updates')
      .on('postgres_changes', 
          { event: 'INSERT', schema: 'public', table: 'bot_transactions', filter: 'bot_type=eq.profiter-two' },
          (payload) => {
  ...
  const handleStartBot = async () => {
    try {
      ...
      enhancedLogger.info("Starting profiter-two bot");
      setIsStarting(true);
      ...
      toast({
        title: "Bot started",
        description: "Profiter Two bot has been started successfully",
      });
  ...
  const handleStopBot = async () => {
    try {
      ...
      enhancedLogger.info("Stopping profiter-two bot");
      setIsStopping(true);
      ...
      toast({
        title: "Bot stopped",
        description: "Profiter Two bot has been stopped successfully",
      });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-crypto-darker text-foreground font-mono flex items-center justify-center">
        <div className="text-neon-pink animate-pulse">Loading bot data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-crypto-darker text-foreground font-mono">
      ...
      <div className="container mx-auto px-4 py-6">
        <h1 className="text-2xl mb-6 font-bold text-neon-pink">Profiter Two Bot Control</h1>
