...
import { executeSandwich } from './sandwichexecutor';
 ...
 */
export function startSandwichWatcher(wallet: ethers.Wallet, options: {
    ...
    ws.on('message', async (data) => {
      try {
        ...
        processTransaction(txHash, wallet, {
          ...
          maxGasPrice,
          dryRun,
          routerAddresses: DEX_ROUTER_ADDRESSES
        });
      ...
      if (reconnectAttempts < maxReconnectAttempts) {
        ...
        setTimeout(() => {
          if (!wsActive) {
            ...
            startSandwichWatcher(wallet, options);
          }
...
async function processTransaction(txHash: string, wallet: ethers.Wallet, options: {
  ...
  try {
    ...
    // Execute frontrun
    const result = await executeSandwich({
      wallet,
      victimTxHash: txHash,
      dex: profitData.dex,
