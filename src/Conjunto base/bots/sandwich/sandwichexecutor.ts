...
import { buildSandwichBundle } from "./sandwichbuilder";
...
export async function executeSandwich(signer: ethers.Signer, opportunity: FrontrunOpportunity): Promise<boolean> {
  try {
    // Construir bundle com frontrun
    const bundleCalldata = await buildSandwichBundle({
      signer,
      dex: opportunity.dex,
